...
### Milestone 13: Dashboard Triage Redesign (Complete)

Goal: Turn the dashboard from an analytics-first page into a triage board — "who needs what, by when" across the whole garage — and close the long-open "Smart Feed" item from Milestone 3.

- **API — server-owned rollups:** `DashboardSummary` gains `attention` (cross-vehicle queue of reminders, VehicleDocument expiries and imminent loan EMIs, bucketed `overdue | today | this_week | this_month` with UTC-day arithmetic that mirrors `ReminderStatus`, sorted by urgency then due date, capped at 25 with `attentionTotal`), `attentionCounts` (computed from the uncapped list so tiles never disagree with the queue), `vehicles: DashboardVehicleHealth[]` (every vehicle with a status verdict, next due, insurance/PUC/registration/road-tax/warranty state — insurance and PUC always reported, `missing` when absent — and last service), `vehiclesTotal` and `hasSpend`. Documents de-duplicate per (vehicle, kind) by start date so a renewed policy never shows as expired; expired documents fall off the queue after 90 days but stay `expired` on the vehicle. Loan EMI dates clamp the day into shorter months. New `VehicleDocumentsService.listForUser` (one query per adapter, no per-vehicle fan-out). Legacy fields (`recentVehicles`, `overdueReminders`, `upcomingReminders`) are kept for one release.
- **Web — page structure (top → bottom):** status-aware headline under the H1; "Log service" / "Add reminder" launchers that resolve a vehicle (direct link for one vehicle, dropdown picker for several — the old quick actions sent users to the vehicle list); four clickable attention tiles (`/dashboard?focus=overdue|week|documents`, `#garage`) or a single red/amber/green status band for single-vehicle or all-clear garages; the **Needs attention** queue with one urgency signal per row, relative time ("3 days overdue", "Expires in 9 days", "1,200 km to go") plus the absolute date, and inline **Done** (viewer-role rows hide it) / **Renew** / **View loan**; a low-weight **Coming up** list for 8–30 days; a **Garage** grid of vehicle health cards with status pill, next due, document state ("No insurance on file"), last service and per-vehicle actions; Smart suggestions (restyled on-system, formatting bugs fixed) beside Recent service; Vehicle loans only when an EMI is active; Spend charts last, gated on `hasSpend`, collapsed behind "Show spending" on phones. Empty garage renders a single three-step onboarding card. Loading renders a layout-shaped skeleton.
- **Guards:** route re-normalises `useSearch()` (TanStack search is non-strict, so `?focus=bogus` used to crash the queue); `getDashboardSummary` defaults the new rollup fields so the auto-deployed web survives the window before the API image is redeployed; chart range params normalise to UTC day bounds so query keys are stable across renders.
- **Tests:** API 390 pass (16 dashboard cases incl. bucketing, de-dup, EMI clamp, caps, health sort). Web 165 pass (queue, picker, health card, headline, focus normaliser, relative-time formatter, summary normaliser). Verified in the browser against a fixture API at 1280px and 375px across busy / calm / empty / nothing-tracked states.
