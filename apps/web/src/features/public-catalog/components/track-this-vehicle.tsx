...
/**
 * The end of every variant page: an offer to track this exact vehicle. It is a
 * plain link carrying the variant as a catalog intent, so it works from static
 * HTML before the app has booted, and it reads auth without requiring it —
 * signed out it goes to registration, signed in straight to the add-vehicle
 * form. Nothing here touches storage; the pages it links to keep the intent.
 */
export function TrackThisVehicle({ page }: TrackThisVehicleProps) {
  ...
  const vehicleName = `${page.make.name} ${page.model.name} ${page.variant.name}`;
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 text-sm leading-6 text-slate-600">
        ...
        falls due. We’ll fill in the make, model and variant for you.
      </p>
