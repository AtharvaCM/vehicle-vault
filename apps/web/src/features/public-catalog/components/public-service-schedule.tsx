...
export function PublicServiceSchedule({ schedule, heading, note }: PublicServiceScheduleProps) {
  return (
    ...
    >
      <h2 className="text-lg font-semibold tracking-tight text-fg" id="service-schedule-heading">
      ...
      {note ? <p className="mt-1 text-sm leading-6 text-fg-2">{note}</p> : null}
      <ul className="mt-4 divide-y divide-line-subtle">
        {schedule.items.map((item) => (
          ...
          >
            ...
            <span className="text-sm text-fg-2">{describeInterval(item)}</span>
          </li>
        ))}
      </ul>
    </section>
