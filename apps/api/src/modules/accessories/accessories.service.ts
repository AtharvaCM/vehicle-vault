...
export class AccessoriesService {
  ...
  async listForVehicle(userId: string, vehicleId: string): Promise<Accessory[]> {
    ...
    const accessories = await this.prisma.accessory.findMany({
      ...
      // "what is on this vehicle" before "what have I bought for it". The nulls
      // clause is load-bearing: Postgres sorts NULLs last on ASC by default,
      // which would put the still-fitted rows at the bottom.
  ...
  async updateAccessory(
    ...
    this.assertCoherentAfterPatch(existing, input);
  ...
  /**
   * The fitment rules describe a whole accessory, so a patch has to be checked
   * against the row it will produce. Validating the patch alone both rejects a
   * bare `{ removedDate }` on an already-fitted row and lets `{ fittedDate: null }`
   * through on a removed one.
   */
  private assertCoherentAfterPatch(existing: AccessoryRow, input: UpdateAccessoryInput) {
    const merged = {
      fittedDate: this.patchedDate(existing.fittedDate, input.fittedDate),
      removedDate: this.patchedDate(existing.removedDate, input.removedDate),
      fittedOdometer:
        input.fittedOdometer !== undefined ? input.fittedOdometer : existing.fittedOdometer,
      removedOdometer:
        input.removedOdometer !== undefined ? input.removedOdometer : existing.removedOdometer,
    };

    const issues = findAccessoryCoherenceIssues(merged);
    ...
    throw new BadRequestException({
      ...
      details: {
        ...
        fieldErrors: issues.reduce<Record<string, string[]>>((acc, issue) => {
          const key = issue.path[0] ?? 'accessory';
  ...
  /** Omitted leaves the stored value; an explicit null clears it. */
  private patchedDate(current: Date | null, patched: string | null | undefined) {
    if (patched === undefined) return current?.toISOString() ?? null;
    return patched;
  }
