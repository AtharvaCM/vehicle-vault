...
**NotifyService**:
`raise(userId, vehicleId, kind, payload, options?)` — resolves template, computes dedup key, upserts the row, and fans out to channels. The single entry point for raising any alert. The default dedup guards unread rows only, so an alert the user read can come back on the next run. Two `RaiseOptions` change that per call: `cooldownDays` skips the raise if the user has had the same alert inside the window, read or not ("same" per the template's `cooldownKey`, or the exact dedup key when it has none), and `inAppOnly` writes the row but skips every external **Channel**. Mechanics only — which alerts use them is the **AlertEngine**'s call.
